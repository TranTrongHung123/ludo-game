...
public static class VerifyRanking
{
    ...
    static RankingController Controller=>UnityEngine.Object.FindFirstObjectByType<RankingController>();
    ...
    static UnityEngine.UI.ScrollRect Scroll=>Controller.transform.Find("Content/RankingCard/ScrollView").GetComponent<UnityEngine.UI.ScrollRect>();
    ...
    {
        var array=new JArray();for(int i=0;i<n;i++)array.Add(new JObject{["rank"]=i+1,["playerId"]="p"+i,["displayName"]=i==0?"Nguyễn Minh Anh":i==1?"Trần Hoàng Nam":i==2?"Lê Bảo Ngọc":"Người kiểm thử "+(i+1),["totalScore"]=150.5m-i*1.5m,["firstPlaceCount"]=30-i});return array;
    }
        ...
        while(!stop.IsCancellationRequested)
        {
            try
            {
                ...
                while(!stop.IsCancellationRequested)
                {
                    ...
                    if(type!="LOGIN"&&type!="RECONNECT")wire&=(string)req["sessionId"]=="ranking-fixture-token";
                    switch(type)
                    {
                        case "LOGIN":response["data"]=new JObject{["sessionId"]="ranking-fixture-token",["profile"]=new JObject{["playerId"]="p1",["displayName"]="Trần Hoàng Nam",["totalScore"]=149,["firstPlaceCount"]=29}};break;
                        ...
                        case "GET_RANKING":
                            ...
                            response["type"]="RANKING_RESULT";response["data"]=new JObject{["entries"]=entries.DeepClone()};
                            if(malformed)response["data"]=new JObject{["entries"]=new JArray(new JObject{["rank"]=1})};
                        ...
                        case "RECONNECT":reconnects++;wire&=(string)req["data"]?["sessionId"]=="ranking-fixture-token";response["type"]="RECONNECT_RESULT";response["data"]=new JObject{["restored"]=true};break;
                        case "LOGOUT":break;
    ...
    static void Capture(int width,int height)
    {
        ...
        try
        {
            ...
            System.IO.File.WriteAllBytes("Documentation/ranking-"+width+"x"+height+"-fixture.png",texture.EncodeToPNG());
            foreach(string path in new[]{"BackButton","RefreshButton","RankingCard","RankingCard/ScrollView","Feedback"})
            {
    ...
    public static async Task<string> Main()
    {
        ...
        try
        {
            ...
            UnityEngine.Object.FindFirstObjectByType<LobbyController>().OpenRanking();await Until(()=>Controller!=null,"Ranking navigation");await Task.Delay(50);
            ...
            var first=Scroll.content.GetChild(0);Check(first.Find("Medal/Rank").GetComponent<TMP_Text>().text=="#1","Rank uses server data");
            Check(first.Find("Score").GetComponent<TMP_Text>().text=="150.5","Fractional total preserved");
            Check(first.Find("Crown").gameObject.activeSelf&&!Scroll.content.GetChild(3).Find("Crown").gameObject.activeSelf,"First rank crown only");
            Check(Scroll.content.GetChild(1).Find("SelfBadge").gameObject.activeSelf,"Current player marked");
            Check(!first.Find("PlayerName").GetComponent<TMP_Text>().richText,"User names are plain text");
            ...
            Check(Text("RankingCard/Empty").gameObject.activeSelf,"Empty state visible");Check(Scroll.content.Cast<Transform>().All(t=>!t.gameObject.activeSelf),"Old rows hidden on empty result");
            entries=Entries(3);entries[0]["rank"]=7;Controller.Refresh();await Until(()=>RefreshButton.interactable,"three rows");
            Check(Scroll.content.GetChild(0).Find("Medal/Rank").GetComponent<TMP_Text>().text=="#7","Client does not infer rank from order");
            ...
            Check(wire,"GET_RANKING empty payload, response type and session over TCP");await session.LogoutAsync();
            SceneManager.LoadScene("RankingScene");await Until(()=>SceneManager.GetActiveScene().name=="LoginScene","unauthenticated guard");Check(true,"Direct unauthenticated entry returns Login");
            return checks+" Ranking checks passed (TCP localhost fixture).";
        }
        finally
        {
