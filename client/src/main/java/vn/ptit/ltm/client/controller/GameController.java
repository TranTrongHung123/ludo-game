...
public final class GameController implements ConnectionAwareController {
    ...
    public void configure(AuthClientService authService, ClientSessionState sessionState) {
        ...
        piecesList.getSelectionModel().selectedItemProperty()
                .addListener((ignored, previous, selected) -> refreshActions());
        render();
    ...
    private void render() {
        ...
        piecesList.getItems().setAll(game.participants().stream()
    ...
    private void refreshActions() {
        if (rollButton == null || moveButton == null || sessionState == null) {
            return;
        }
        GameStateDto game = sessionState.gameState().orElse(null);
        PieceDto selected = piecesList.getSelectionModel().getSelectedItem();
